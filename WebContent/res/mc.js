...
function quantityHandler(request){
	if ((request.readyState == 4) && (request.status == 200)){
		...
		for (var qty in elements){
			...
			row.insertCell(1).innerHTML = "<button name='qtyDec' label='qtyDec'	class='qty' onclick='dec(this)'>-</button>";
			row.insertCell(2).innerHTML = info[1].trim();
			row.insertCell(3).innerHTML = "<button name='qtyInc' label='qtyInc'	class='qty' onclick='inc(this)'>+</button>";
			row.insertCell(4).innerHTML = "$" + (parseFloat(info[2].trim()) * parseFloat(info[1].trim()));
		}
